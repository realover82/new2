...
def create_simple_bar_chart(df: pd.DataFrame, key_prefix: str, jig_separated: bool):
    ...
    # if jig_separated:
    #     # Jig와 Date, Test를 결합하여 고유한 X축 레이블 생성
    #     df_chart_base['Test_ID'] = (
    #         df_chart_base['Date'].astype(str) + " / " + 
    #         df_chart_base['Jig'].astype(str) + " / " + 
    #         df_chart_base['Test']
    ...
    #     x_axis_label = '날짜 / Jig / 테스트 항목'
    # else:
    #     # Test 항목만 사용하여 합산된 차트 생성 (기본)
    #     df_chart_base = df_chart_base.groupby('Test')[['미달 (Under)', '초과 (Over)', 'Failure']].sum().reset_index()
    #     df_chart_base['Test_ID'] = df_chart_base['Test']
    #     x_axis_label = '테스트 항목 (전체 합산)'

    # # 2. X축을 'Test_ID'로, Y축을 불량 항목으로 설정
    # df_chart = df_chart_base.set_index('Test_ID')[['미달 (Under)', '초과 (Over)', 'Failure']]

    # 2. X축을 'Test_ID'로, Y축을 불량 항목으로 설정
    # Jig별 구분 (분리가 필요한 경우에만)
    if jig_separated:
        df_chart_base = df_chart_base.set_index('Test_ID')
    else:
        # Jig와 Date를 합산하여 Test 항목별 합계만 보여줍니다.
        df_chart_base = df_chart_base.groupby('Test_ID')[['미달 (Under)', '초과 (Over)', 'Failure']].sum().reset_index()
        df_chart_base = df_chart_base.set_index('Test_ID')
        x_axis_label = '날짜 / Jig / 테스트 항목 (합산)'
    ...
    # 3. Streamlit의 기본 막대 차트 위젯을 사용하여 출력
    # st.bar_chart는 인덱스(Test_ID)를 X축으로 사용하고, 컬럼들을 Y축의 누적 값으로 사용합니다.
    st.bar_chart(df_chart_base[['미달 (Under)', '초과 (Over)', 'Failure']]) 
